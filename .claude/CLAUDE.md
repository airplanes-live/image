...
SSH ships enabled but password-login-off by default (`stage-airplanes/00-prep/02-run.sh` drops `90-airplanes.conf` with `PasswordAuthentication no` plus an `AuthorizedKeysFile .ssh/authorized_keys /etc/ssh/authorized_keys.d/%u`). Per-device opt-in for the `pi` account has three byte-identical paths: rpi-imager Edit Settings, the webconfig `apl-ssh` helper, and `airplanes-config.txt` (`SSH_PASSWORD` / `SSH_PUBKEY`). The boot-config flow lives in `airplanes-first-run`'s `apply_ssh_password` / `apply_ssh_pubkey`: `SSH_PASSWORD` (min 12 chars) sets the pi password via `chpasswd` and writes the Match-scoped `99-airplanes-ssh-pi.conf` snippet (shared verbatim with `apl-ssh`); `SSH_PUBKEY` overwrites `/etc/ssh/authorized_keys.d/pi`. Both are synthetic — stripped from `BOOT_CFG` so they never reach feed.env — and `SSH_PASSWORD`'s cleartext is redacted from the FAT source at apply time, decoupled from the success rename. The `pi` NOPASSWD sudo grant (`stage-airplanes/06c-grant-sudo/`) is dormant until one of these enables login, then `pi` reaches root by design.
